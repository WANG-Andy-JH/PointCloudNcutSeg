...
d = d + offset * 2;
dr = dr + offset;
%通过获取dr的列(=1)并沿 0（中间） 指定的对角线放置它们，来创建一个 m×n 稀疏矩阵 S
...
%求解特征值 Afun：@mex_w_times_x_symmetric,size(P,1)，后面是参数，创建稀疏矩阵大小
%vbar包含每一列的特征向量，切归一化，s为各列特征向量对应特征值
[vbar,s] = eigs(@mex_w_times_x_symmetric,size(P,1),nbEigenValues,'LA',options,tril(P)); 
